...
namespace Method1.Services.Versioning
{
    public class ApiVersioningSelector : DefaultHttpControllerSelector
    {
        ...
        private string GetVersionFromMediaType(HttpRequestMessage request)
        {
            ...
            var regularExpression = new Regex(@"application\/vnd\.meijer\.([a-z]+)\.([a-z]+)\-v(\d.\d)\+json", RegexOptions.IgnoreCase);

            //var regularExpression = new Regex(@"^application\/vnd.meijer..*-v(\d.\d)\+json", RegexOptions.IgnoreCase);
            ...
            foreach (var mime in acceptHeader)
            {
                ...
                {
                    return match.Groups[3].Value.Replace('.', '_');
                }
            }
            return "2_0"; //if not mime type return the API latest version
        }
